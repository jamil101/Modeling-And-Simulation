...
#define BOT_TOKEN "8435066468:AAGJiyvhtlUxYoXluewx7oV0zcy53vYaXaM" // Replace with your Telegram Bot Token
#define CHAT_ID "6436948731"                                       // Replace with your Chat ID
...
#define TELEGRAM_ALERT_INTERVAL 60000 // Check for alerts every minute
#define TELEGRAM_POLL_INTERVAL 2000   // Check for new messages every 2 seconds
...
// Telegram message offset tracking
int lastMessageId = 0;
...
void processTelegramCommand(String text, String chat_id);
...
void updateRainStatus()
{
  ...
  // Only print and notify when status changes
...
void checkAndReportFaults()
{
  ...
  // Check pump stuck condition (pump running too long)
  ...
  // Update active faults and send summary if new fault appears
  if (hasFault)
  {
...
void processTelegramCommand(String text, String chat_id)
{
  ...
  {
    response = "🌱 SMART FARM COMMANDS:\n\n";
    response += "/status - Current system status\n";
    response += "/pump_on - Turn pump ON (switches to manual mode)\n";
    response += "/pump_off - Turn pump OFF (switches to manual mode)\n";
    response += "/auto - Switch to AUTO mode\n";
    response += "/manual - Switch to MANUAL mode\n";
    response += "/help - Show this help\n";
  }
  else if (text == "/status")
  {
    sendStatusUpdate();
    return; // Already sends the full status, no need for additional response
  }
  else if (text == "/pump_on")
  {
    // Check critical water level safety
    if (waterLevel < WATER_CRITICAL)
    {
      response = "❌ Cannot turn pump ON: CRITICAL WATER LEVEL!\n";
      response += "Water level: " + String(waterLevel) + " (min: " + String(WATER_CRITICAL) + ")";
    }
    else
    {
      manualMode = true;
      manualPumpRequest = true;
      response = "✅ Pump turned ON (Manual Mode)\n";
      response += "Reason: Remote command";
      sendTelegramMessage("💧 REMOTE PUMP ON\nPump activated via Telegram command", false);
    }
  }
  else if (text == "/pump_off")
  {
    ...
    response = "✅ Pump turned OFF (Manual Mode)\n";
    response += "Reason: Remote command";
    sendTelegramMessage("💧 REMOTE PUMP OFF\nPump deactivated via Telegram command", false);
  }
  else if (text == "/auto")
  {
    manualMode = false;
    response = "✅ System set to AUTO mode\n";
    response += "Pump will be controlled automatically based on sensors";
    sendTelegramMessage("🔄 MODE CHANGED TO AUTO\nSystem now running automatic control", false);
  }
  else if (text == "/manual")
  {
    manualMode = true;
    response = "✅ System set to MANUAL mode\n";
    response += "Pump can be controlled via /pump_on and /pump_off commands";
    sendTelegramMessage("🔄 MODE CHANGED TO MANUAL\nPump now under manual control", false);
  }
  else
  {
...
void setup()
{
  ...
  Serial.println("Telegram alerts and remote control enabled");
}
